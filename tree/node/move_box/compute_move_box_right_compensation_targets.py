...
class ComputeMoveBoxRightCompensationTargets(TimedMockAction):
    ...
    def __init__(self, name, config_label, ros_node, params):
        ...
        self.right_edge_key = str(
            params.get("right_edge_key", "move_box_right_edge_point")
        ).strip()
        self.box_axes_key = str(
            params.get("box_axes_key", "move_box_latest_box_axes")
        ).strip()
        self.left_pull_key = str(
            params.get("left_pull_key", "move_box_left_pull_target")
        ).strip()
        self.target_keys = {
            "moved_right_edge": str(
                params.get(
                    "moved_right_edge_key",
                    "move_box_right_compensation_moved_edge",
                )
            ).strip(),
            "right_above": str(
                params.get("right_above_key", "move_box_right_compensation_above_edge")
            ).strip(),
            "right_below": str(
                params.get("right_below_key", "move_box_right_compensation_below_edge")
            ).strip(),
            "right_lift": str(
                params.get("right_lift_key", "move_box_right_compensation_lift_target")
            ).strip(),
    ...
    def update(self):
        """按左手外拉位移补偿右侧箱体边缘并生成后续目标。"""
        if self.should_use_mock_execution():
            return self.update_mock_result()

        right_edge_point = (
            self.blackboard.get(self.right_edge_key)
            if self.blackboard.exists(self.right_edge_key)
            else None
        )
        box_axes = (
        ...
        left_pull_target = (
            self.blackboard.get(self.left_pull_key)
            if self.blackboard.exists(self.left_pull_key)
            else None
        )
        if right_edge_point is None or box_axes is None or left_pull_target is None:
            self.ros_node.get_logger().error(
                f"[{self.config_label}] 缺少右手目标计算所需抓取数据"
            )
